...
class Event2RDF(object):
    ...
    def add_event_to_graph(self, event):
        ...
        if actor != "":
            ...
            actor_node.add(DOCKEVENT.actorId, Literal(actor_id, datatype=XSD.string))
            actor_attributes = actor.get("Attributes", "")
            if actor_attributes != "":
                image = actor_attributes.get("image", "")
                actor_node.add(DOCKEVENT.actorId, Literal(image, datatype=XSD.string))
                ...
                actor_node.add(DOCKEVENT.swarmIpPort, Literal(swarm_ip_port, datatype=XSD.string))
    ...
    def serialize(self):
        return self.store.serialize(format="turtle")
...
for event_bytestring in event_listener.listen():
    ...
    print(e2rdf.serialize().decode("utf-8"))
    break
